        ...
        for G in graph_list:
            ...
            try:
                ...
                if "optimal_value" in result:
                    values.append(-result["optimal_value"])  # Минус, так как Optuna минимизирует
                else:
...
for n in tqdm(range(5, 11), desc="Размеры графа"):
    ...
    study.optimize(make_objective(n, graph_list), n_trials=10)

    results[n] = {
        "best_params": study.best_params,
        "best_value": -study.best_value,  # инвертируем обратно
    }

with open("qaoa_optuna_results.json", "w") as f:
